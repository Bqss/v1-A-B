...
const Tabs: TabsType = ({ activeTab = 0, setActiveTab, children, className = "" }) => {
  return (
    <TabsContext.Provider value={{ activeTab, setActiveTab }}>
      <div className={classNames(className, 'flex flex-col md:flex-row items-start gap-10')}>
        {children}
      </div>
...
const List: FC<ListProps> = ({ children, className = '' }) => {
  ...
  return (
    ...
    <div className='relative flex flex-col-reverse  md:flex-row h-full w-full md:w-auto '>
      ...
      <ul className={classNames(className, "relative flex md:flex-col w-full overflow-x-auto scrollbar_hidden ")} ref={listRef} >{children}</ul>
    </div>
...
const Tab: FC<TabProps> = ({ children, value, className = '' }) => {
  ...
  return (
    <li className={classNames(className, 'w-full')} >
      <button className={classNames('px-8 py-2 w-full text-xs sm:text-sm text-left hover:bg-themecolor/10 hover:text-themecolor transition duration-100', (activeTab == value ? "bg-themecolor/10 text-themecolor" : ''))} onClick={setActiveTab && (() => setActiveTab(value))}>
        {children}
      </button>
    </li>
  )
